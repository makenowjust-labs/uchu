...
/** Get is a getter. */
trait Get[A] extends (N => Option[A]) {
  def apply(n: N): Option[A]
}
...
object Get {
  ...
  def apply[A](f: N => Option[A]): Get[A] = new Get[A] {
    def apply(k: N): Option[A] = f(k)
  }
  ...
  val unit: Get[Unit] = Get(k => if (k == N.Zero) Some(()) else None)

  /** Gets a [[Boolean]] value from an index. */
  val boolean: Get[Boolean] = Get { k =>
    if (k == N.Zero) Some(false)
    else if (k == N.One) Some(true)
    else None
  }
  ...
  val byte: Get[Byte] =
    Get(k => if (k < N.Two ** 8) bigInt(k).map(_.toByte) else None)

  /** Gets a [[Short]] value from an index. */
  val short: Get[Short] =
    Get(k => if (k < N.Two ** 16) bigInt(k).map(_.toShort) else None)

  /** Gets a [[Int]] value from an index. */
  val int: Get[Int] =
    Get(k => if (k < N.Two ** 32) bigInt(k).map(_.toInt) else None)

  /** Gets a [[Long]] value from an index. */
  val long: Get[Long] =
    Get(k => if (k < N.Two ** 64) bigInt(k).map(_.toLong) else None)

  /** Gets a pair of values from an index. */
  def tuple2[A, B](gx: Get[A], cx: Card, gy: Get[B], cy: Card): Get[(A, B)] = {
    val c = cx * cy

    def finFin(nx: N, ny: N): Get[(A, B)] = {
      ...
      Get { k =>
        if (k < upper) {
          ...
          val kx = nx - 1 - jx
          val ky = (if (landscape) N.One else (max - min) + 1) + j + jx
          Some(gx(kx).get, gy(ky).get)
        } else None
      }
    }

    def finInf(n: N, landscape: Boolean): Get[(A, B)] = {
      val upper = n * (n + 1) / 2
      Get { k =>
        if (k < upper) {
          val d = diagonal(k)
  ...
  def set[A](g: Get[A], c: Card): Get[Set[A]] = Get { k =>
    ...
    else Some(k.bits.zipWithIndex.collect { case (true, k) => g(N(k)).get }.toSet)
  }
    ...
    Get { k =>
      if (k == N.Zero) Some(Map.empty)
        ...
        gCons(k - 1).map { case (xys, y) =>
          val map1 = xys.zipWithIndex.collect { case (Some(x), k) => (gx(N(k)).get, x) }.toMap
          val map2 = Map(gx(N(xys.size)).get -> y)
          map1 ++ map2
        }
    }
  }
  ...
  def option[A](g: Get[A]): Get[Option[A]] =
    Get(k => if (k == N.Zero) Some(None) else g(k - 1).map(Some(_)))

  /** Gets an either value from an index. */
  def either[A, B](gx: Get[A], cx: Card, gy: Get[B], cy: Card): Get[Either[A, B]] = {
    def smallLeft(nx: N): Get[Either[A, B]] = Get { k =>
      if (k >= nx * 2) gy(k - nx).map(Right(_))
      else {
        val (d, r) = k /% N.Two
        if (r == N.Zero) gx(d).map(Left(_)) else gy(d).map(Right(_))
      }
    }

    def smallRight(ny: N): Get[Either[A, B]] = Get { k =>
  ...
  /** Computes a diagonal index from an index.
    ...
    */
  private def diagonal(n: N): N = (N.sqrt(n * 8 + 1) - 1) / 2
